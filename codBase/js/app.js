var calculadora = {
	...
	init: (function(){
		this.EventosFormatoBotones(".tecla");
		this.EventosaFuncion();
	}),

	//Eventos de formato de botones
	EventosFormatoBotones: function(selector){
		var x = document.querySelectorAll(selector);
		for (var i = 0; i<x.length;i++) {
			x[i].onmouseover = this.eventoReduceBoton;
			x[i].onmouseleave = this.eventoAmpliaBoton;
		};
	},
	...
	eventoAmpliaBoton: function(event){
		calculadora.AmpliaBoton(event.target);
	},
	...
	AmpliaBoton: function(elemento){
		var x = elemento.id;
		if (x=="0" || x=="1" || x=="2" || x=="3" || x=="igual" || x=="punto" ) {
